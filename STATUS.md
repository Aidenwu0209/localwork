...
| 状态 | 内容 |
|---|---|
| **已完成** | G0+M+D **33/33 accept**;P3.3 README;P3.5 licenses;P3.6 哨兵;P3.7 perceive |
| **阻塞** | **P3.1** ROCm 消融收尾(brain×MTP×并发)——服务器 SSH `:30147` 曾不通 |
| **未开始** | **P3.4** 演示视频; **P3.2** Grafana |
| **可砍** | MCP / 音频 / MarkItDown / 日报多 Agent UI |

**下一优先:P3.1**(SSH 通后)→ P3.4 → P3.2。
...
3. `TASKBOARD.json`(领 P3.1 / P3.2 / P3.4)
...
服务器:`root@36.150.116.200 -p 30147`。起模型前 `rocm-smi`;brain 与 Dolphin 共存用 Q6_K。
...
| P3.1 | **blocked** | 小模型数字已在 `benchmarks.md §2`;brain 矩阵未测 |
...
1. 勿重做 33+4 已 accept 项。
2. 起 GPU 前 `rocm-smi`,勿 OOM Dolphin。
